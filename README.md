...
| 4 | **Lean 4 Formalization** | ✅ fatto | Definizioni rigorose (P, NP, riduzioni, NP-completezza) e **22 teoremi** verificati dal kernel. Nucleo astratto (zero assiomi): P ⊆ NP, riduzioni come preordine, collasso di Cook **e la sua caratterizzazione** (`L∈P ↔ P=NP`), barriera di relativizzazione. **Verso Cook–Levin**: un `Model` concreto «senza limiti» in cui **P = NP per forza bruta** (`collapse_world_exists` — il framework non è vacuo), e **CNF-SAT concreto** con forma-NP e una riduzione verificata (`eval_rename`). Zero `sorry`; il livello concreto usa solo `propext`/`Quot.sound`, mai `Classical.choice`. Vedi `formalization/`. |
...
in modo semplice, le tre barriere, cosa fanno gli otto moduli e i risultati raggiunti.
