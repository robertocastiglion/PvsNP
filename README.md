...
| 4 | **Lean 4 Formalization** | ✅ fatto | Definizioni rigorose (P, NP, riduzioni, NP-completezza) e **34 teoremi** verificati dal kernel. Nucleo astratto (zero assiomi): P ⊆ NP, riduzioni come preordine, collasso di Cook **e la sua caratterizzazione** (`L∈P ↔ P=NP`), barriera di relativizzazione. **Verso Cook–Levin**: un `Model` concreto «senza limiti» in cui **P = NP per forza bruta** (`collapse_world_exists`), **CNF-SAT concreto** con forma-NP e una riduzione verificata, e **il cuore di Cook–Levin** (`canonical_correct`: vincolo locale → CNF). **Nucleo di Razborov–Rudich** (`NaturalProofs.lean`): le funzioni booleane come tavole di verità (conteggio verificato 2^(2^n)), `Useful` (∀ facili rifiutate, caso peggiore), `Large` (conteggio costruttivo), `Constructive` (astratto come `Model.PolyDec`), e la **barriera come teorema** (`rr_barrier`: se la crittografia è sicura, nessuna prova naturale esiste) col cuore crittografico per ipotesi. Zero `sorry`; solo `propext`/`Quot.sound`, mai `Classical.choice`. Vedi `formalization/`. |
...
| 12 | **Algorithmic Method Lab** | ✅ fatto | Il **metodo di Ryan Williams** (NEXP ⊄ ACC⁰, 2011): «un algoritmo SAT più veloce della forza bruta ⇒ un lower bound» — l'unico approccio che **aggira tutte e tre le barriere**. Cuore eseguibile (esatto): i circuiti strutturati hanno **polinomi sparsi**, quindi si conta #SAT su tutti i 2ⁿ input *senza enumerarli* (AND: 4096× più veloce; OR denso: nessun guadagno). Più la **catena win-win** e la soglia (serve speedup **super-polinomiale**). **#SAT esplicito per profondità-2** (`depth2.py`): un solver per DNF via inclusione–esclusione sui termini, O(2^m) invece di O(2^n), **più veloce della forza bruta** sul regime strutturato — con **tempi reali misurati** (a m=4: forza bruta 18ms→15s da n=10 a n=20, l'algoritmo resta ~0.1ms; speedup misurato ~38000×, conteggi verificati). Onestà: eseguiamo il meccanismo; Easy Witness Lemma, PCP, gerarchia e l'algoritmo ACC⁰ vero (2^{n−n^δ}) sono **citati**. Genera 1 grafico SVG. `pnp_lab/algorithmic_method/`. |
